...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        if serial_ports:
            ...
            parser.error('No serial ports detected')
            default_serial_port = ''
        ...
        parser.add_argument('-l', '--load-address', default=0x94100000, type=lambda x: int(x, 16),
                            help='Upload firmware to memory address')
        parser.add_argument('-s', '--serial', type=serial_interface,
                            default=default_serial_port, help='Serial port')
        parser.add_argument('firmware', type=argparse.FileType('r'), nargs='?', default=None)
